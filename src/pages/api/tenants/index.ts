...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    // ✅ Ensure session exists and user object is present
    ...
    switch (req.method) {
        case "GET":
            try {
                ...
                if (session.role === "admin") {
                    ...
                    tenants = await Tenant.find({ users: session.id }); // Users & managers see assigned tenants
                }
        ...
        case "POST":
            try {
                if (session.role !== "admin") {
                    return res.status(403).json({ message: "Only admins can create tenants" });
                }
